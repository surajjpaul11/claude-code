...
description: Pre-restart checklist for Docker containers — save code, update memory.md, record loops, ensure dependencies tracked. Use before container stops.
---
...
- Ensure all `/loop` jobs are in `loops.md` with job ID, interval, prompt, date
...
- Record servers/background processes in `memory.md`: what, command, port
